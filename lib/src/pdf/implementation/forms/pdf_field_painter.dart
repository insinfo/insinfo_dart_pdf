...
class FieldPainter {
  ...
  void drawRectangularControl(PdfGraphics graphics, PaintParams params) {
    graphics.drawRectangle(
      bounds: params.bounds ?? Rect.zero,
      brush: params.backBrush,
    );
    drawBorder(
      graphics,
      params.bounds,
    ...
    switch (state) {
      ...
      case PdfCheckFieldState.checked:
        if (font == null) {
          ...
          size =
              (paintParams.bounds!.width > paintParams.bounds!.height)
    ...
    if (borderPen != null) {
      if (borderWidth! > 0 && !borderPen.color.isEmpty) {
        if (style == PdfBorderStyle.underline) {
          graphics.drawLine(
            borderPen,
            Offset(bounds!.left, bounds.top + bounds.height - borderWidth / 2),
            Offset(
    ...
    switch (paintParams.style) {
      ...
      case PdfBorderStyle.inset:
        switch (state) {
          ...
          case PdfCheckFieldState.unchecked:
            leftTopPen = PdfPen(
              PdfColor(255, 128, 128, 128),
              width: borderWidth,
            );
            rightBottomPen = PdfPen(
              PdfColor(255, 192, 192, 192),
              width: borderWidth,
            );
            break;
        }
  ...
  void drawListBox(
    ...
    for (int index = 0; index < items.count; index++) {
      ...
      final PdfRectangle itemTextBound = PdfRectangle(
        point.dx,
        point.dy,
        width - point.dx,
        font.height,
      );
      PdfGraphicsHelper.getHelper(graphics).layoutString(
        value,
        font,
  ...
  void drawTextBox(
    ...
    if (multiline) {
      ...
      final double ascent = PdfFontHelper.getHelper(
        font,
      ).metrics!.getAscent(format);
      final double descent = PdfFontHelper.getHelper(
        font,
