...
class PdfLineAnnotation extends PdfAnnotation {
  ...
  PdfLineAnnotation(
    ...
    _helper = PdfLineAnnotationHelper(
      this,
      linePoints,
      text,
      color: color,
  ...
  set lineIntent(PdfLineIntent value) {
    ...
    if (!helper.isLoadedAnnotation) {
      ...
      helper.dictionary!.setName(
        PdfName(PdfDictionaryProperties.it),
        helper.getEnumName(value),
      );
    }
  }
...
class PdfLineAnnotationHelper extends PdfAnnotationHelper {
  ...
  PdfLineAnnotationHelper(
    ...
    dictionary!.setProperty(
      ...
      PdfName(PdfDictionaryProperties.line),
    );
  }
  ...
  void _savePdfLineDictionary() {
    ...
    if (lineStyle!.elements.isNotEmpty) {
      lineStyle!.insert(
        0,
        PdfName(helper.getEnumName(lineAnnotation.beginLineStyle)),
      );
      lineStyle!.insert(
        1,
        PdfName(helper.getEnumName(lineAnnotation.endLineStyle)),
      );
    } else {
      lineStyle!.add(
        PdfName(helper.getEnumName(lineAnnotation.beginLineStyle)),
    ...
    dictionary[PdfDictionaryProperties.c] = PdfColorHelper.toArray(
      lineAnnotation.color,
    );

    dictionary.setProperty(
      PdfDictionaryProperties.it,
      PdfName(helper.getEnumName(lineAnnotation.lineIntent)),
    );

    dictionary.setProperty(
      PdfDictionaryProperties.lle,
      PdfNumber(lineAnnotation.leaderLineExt),
    );
    ...
    dictionary.setProperty(
      ...
      PdfBoolean(lineAnnotation.lineCaption),
    );
  ...
  void _flattenAnnotation(PdfPage? page, PdfTemplate? appearance) {
    ...
    final PdfDictionary dictionary = helper.dictionary!;
    if (helper.isLoadedAnnotation) {
      final bool isContainsAP = dictionary.containsKey(
        PdfDictionaryProperties.ap,
      );
  ...
  PdfTemplate? _createAppearance() {
    ...
    final PdfPen mBorderPen = PdfPen(
      lineAnnotation.color,
      width: lineAnnotation.border.width,
    );
    if (lineAnnotation.border.borderStyle == PdfBorderStyle.dashed) {
    ...
    if (linePoints.length == 4) {
      ...
      final List<double> startingPoint = helper.getAxisValue(
        x1y1,
        lineAngle + 90,
        line,
      );
      final List<double> endingPoint = helper.getAxisValue(
        x2y2,
        lineAngle + 90,
        line,
      );
      ...
      final List<double> middlePoint1 = helper.getAxisValue(
        startingPoint,
        angle,
        centerWidth,
      );
      final List<double> middlePoint2 = helper.getAxisValue(
        endingPoint,
        angle + 180,
        centerWidth,
      );
      ...
      final List<double> beginLineExt = helper.getAxisValue(
        startingPoint,
        lineAngle + 90,
        lineAnnotation.leaderLineExt.toDouble(),
      );
      graphics.drawLine(
        mBorderPen,
      ...
      //Set leader line
      final List<double> beginLeaderLine = helper.getAxisValue(
        startingPoint,
        lineAngle - 90,
        leadLine,
      );
      ...
      final List<double> endLeaderLine = helper.getAxisValue(
        endingPoint,
        lineAngle - 90,
        leadLine,
      );
      ...
      if (lineAnnotation.lineCaption) {
        ...
        final List<double> centerPoint = helper.getAxisValue(
          startingPoint,
          angle,
          midpoint,
        );
        final List<double> captionPosition = _getCaptionPosition(
          caption,
          centerPoint,
          angle,
          mFont,
        ...
        graphics.drawString(
          ...
          mFont,
          brush: backBrush,
          bounds: Rect.fromLTWH(-lineWidth / 2, 0, 0, 0),
        );
      }
    }
  ...
  List<double> _getCaptionPosition(
    ...
    if (helper.isLoadedAnnotation) {
      final bool isContainsMeasure = helper.dictionary!.items!.containsKey(
        PdfName(PdfDictionaryProperties.measure),
      );
      final double length =
          caption == 'Top'
              ? isContainsMeasure
      ...
      captionPosition = helper.getAxisValue(
        centerPoint,
        angle + 90,
  ...
  PdfLineIntent _obtainLineIntent() {
    ...
    if (helper.dictionary!.containsKey(PdfDictionaryProperties.it)) {
      final PdfName lineintent =
          helper.crossTable.getObject(
                helper.dictionary![PdfDictionaryProperties.it],
              )!
              as PdfName;
  ...
  List<int> _obtainLinePoints() {
    ...
    if (!PdfAnnotationHelper.getHelper(lineAnnotation).isLoadedAnnotation) {
      ...
      if (PdfAnnotationHelper.getHelper(
        lineAnnotation,
      ).dictionary!.containsKey(PdfDictionaryProperties.l)) {
        linePoints =
            PdfCrossTable.dereference(
  ...
  bool _obtainLineCaption() {
    ...
    if (PdfAnnotationHelper.getHelper(
      lineAnnotation,
    ).dictionary!.containsKey(PdfDictionaryProperties.cap)) {
      final PdfBoolean lCap =
          PdfAnnotationHelper.getHelper(
                lineAnnotation,
              ).dictionary![PdfDictionaryProperties.cap]!
              as PdfBoolean;
      lCaption = lCap.value!;
    }
    return lCaption;
  ...
  int _obtainLeaderExt() {
    ...
    if (PdfAnnotationHelper.getHelper(
      lineAnnotation,
    ).dictionary!.containsKey(PdfDictionaryProperties.lle)) {
      final PdfNumber lExt =
          PdfAnnotationHelper.getHelper(
                lineAnnotation,
              ).dictionary![PdfDictionaryProperties.lle]!
              as PdfNumber;
  ...
  int _obtainLeaderOffset() {
    ...
    if (PdfAnnotationHelper.getHelper(
      lineAnnotation,
    ).dictionary!.containsKey(PdfDictionaryProperties.llo)) {
      final PdfNumber lOffset =
          PdfAnnotationHelper.getHelper(
                lineAnnotation,
              ).dictionary![PdfDictionaryProperties.llo]!
              as PdfNumber;
  ...
  PdfArray? _obtainLineStyle() {
    ...
    if (helper.dictionary!.containsKey(PdfDictionaryProperties.le)) {
      array =
          helper.crossTable.getObject(
                helper.dictionary![PdfDictionaryProperties.le],
              )
              as PdfArray?;
    }
    return array;
  ...
  PdfLineCaptionType _obtainCaptionType() {
    ...
    if (PdfAnnotationHelper.getHelper(
      lineAnnotation,
    ).dictionary!.containsKey(PdfDictionaryProperties.cp)) {
      final PdfName cType =
          PdfAnnotationHelper.getHelper(
                lineAnnotation,
              ).dictionary![PdfDictionaryProperties.cp]!
              as PdfName;
