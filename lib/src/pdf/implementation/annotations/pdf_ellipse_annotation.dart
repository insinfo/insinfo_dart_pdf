...
class PdfEllipseAnnotation extends PdfAnnotation {
  ...
  PdfEllipseAnnotation(
    ...
    _helper = PdfEllipseAnnotationHelper(
      this,
      bounds,
      text,
      color: color,
...
class PdfEllipseAnnotationHelper extends PdfAnnotationHelper {
  ...
  void _flattenAnnotation(PdfPage? page, PdfTemplate? appearance) {
    ...
    if (helper.isLoadedAnnotation) {
      final bool isContainsAP = helper.dictionary!.containsKey(
        PdfDictionaryProperties.ap,
      );
      if (appearance == null) {
        if (isContainsAP) {
          PdfDictionary? appearanceDictionary =
              PdfCrossTable.dereference(
                    helper.dictionary![PdfDictionaryProperties.ap],
                  )
                  as PdfDictionary?;
  ...
  void _drawAppearance(
    ...
    if (PdfAnnotationHelper.getHelper(
      annotation,
    ).dictionary!.containsKey(PdfDictionaryProperties.rd)) {
      final PdfArray? rdArray =
          PdfCrossTable.dereference(
  ...
  void _populateBezierPoints(
    ...
    if (currentIteration < 2) {
      ...
      final Offset midPoint1 = Offset(
        (ctrl1.dx + ctrl2.dx) / 2,
        (ctrl1.dy + ctrl2.dy) / 2,
      );
      final Offset midPoint2 = Offset(
        (ctrl2.dx + ctrl3.dx) / 2,
        (ctrl2.dy + ctrl3.dy) / 2,
      );
