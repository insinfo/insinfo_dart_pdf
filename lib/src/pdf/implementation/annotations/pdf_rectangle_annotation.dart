...
class PdfRectangleAnnotationHelper extends PdfAnnotationHelper {
  ...
  PdfRectangleAnnotationHelper._(
    this.rectangleAnnotation,
    PdfDictionary dictionary,
    PdfCrossTable crossTable,
  ) : super(rectangleAnnotation) {
  ...
  PdfTemplate? _createAppearance() {
    ...
    if (helper.isLoadedAnnotation) {
      if (rectangleAnnotation.setAppearance) {
        ...
        final PdfTemplate template = PdfTemplateHelper.fromRect(
      ...
      final Rect nativeRectangle = Rect.fromLTWH(
        0,
        0,
        rectangleAnnotation.bounds.width,
        rectangleAnnotation.bounds.height,
  ...
  Map<String, dynamic> _calculateCloudBorderBounds() {
    ...
    if (!dictionary.containsKey(PdfDictionaryProperties.rd) &&
      ...
      if (dict.containsKey(PdfDictionaryProperties.s)) {
        borderStyle = helper.getEnumName(
          (dict[PdfDictionaryProperties.s]! as PdfName).name,
        );
      }
      ...
      final PdfDictionary? bSDictionary =
          PdfCrossTable.dereference(
                PdfAnnotationHelper.getHelper(
                  rectangleAnnotation,
                ).dictionary![PdfDictionaryProperties.bs],
              )
              as PdfDictionary?;
  ...
  void _drawAppearance(
    ...
    if (radius > 0) {
      ...
      for (
        ...
        points.add(
          Offset(
            PdfPathHelper.getHelper(graphicsPath).points[i].dx,
