...
class IcpBrasilPolicyEngine {
  ...
  PolicyValidationResult validatePolicy(String policyOid, DateTime signingTime) {
    if (lpa == null) {
    ...
    for (final PolicyInfo info in lpa!.policyInfos) {
        // We assume policyName or fieldOfApplication *contains* the OID or we need to parse the URI.
        // In the Demoiselle code, matching is done by PolicyURI usually.
        // For this implementation, we will check if the OID matches the policy name.
        // Real implementation requires robust OID matching.
        
        // Simplification: Check if policyName (DirectoryString) == policyOid
        if (info.policyName == policyOid) {
           return _checkPeriod(info, signingTime);
        }
    }
  ...
  PolicyValidationResult _checkPeriod(PolicyInfo info, DateTime time) {
      ...
      if (time.isBefore(notBefore)) {
          return PolicyValidationResult(isValid: false, error: 'Signature time before policy validity');
      }
      if (notAfter != null && time.isAfter(notAfter)) {
          return PolicyValidationResult(isValid: false, error: 'Signature time after policy validity');
      }
      if (revoked != null && time.isAfter(revoked)) {
         return PolicyValidationResult(isValid: false, error: 'Policy was revoked before signature time');
      }
  ...
  PolicyValidationResult _validateHardcoded(String oid, DateTime time) {
      ...
      return PolicyValidationResult(isValid: false, error: 'Unknown Policy OID (LPA missing)');
  }

  /// Validates if the digest algorithm is allowed by the policy.
  PolicyValidationResult validateAlgorithm(String policyOid, String digestAlgorithmOid, [DateTime? signingTime]) {
       ...
       if (policyOid.startsWith('2.16.76.1.7.1.1.2')) { 
          ...
          if (digestAlgorithmOid == '1.3.14.3.2.26') {
               return PolicyValidationResult(isValid: false, error: 'SHA-1 is not allowed for $policyOid');
          }
          
          return PolicyValidationResult(isValid: false, error: 'Algorithm $digestAlgorithmOid not allowed for policy $policyOid (Expected SHA256)');
       }
       
       // Fallback for older or other policies
       return PolicyValidationResult(isValid: true, warning: 'Algorithm validation skipped (Policy XML missing)');
  }
}
