  ...
  group('PKI Simulation & PDF Signing', () {
    ...
    Future<Uint8List> generateSignedPdf() async {
      ...
      final signature = PdfSignature(
        ...
        timestampServer: TimestampServer(
            Uri.parse('http://localhost:$serverPort/timestamp')),
      );

      final page = document.pages[0];
      final field = PdfSignatureField(page, 'Signature1',
          signature: signature, bounds: Rect.fromLTWH(0, 0, 200, 50));
    ...
    test('Validate Signature Chain Trust (Success Case)', () async {
      ...
      expect(sig.revocationStatus.status, equals('good'),
          reason: 'Revocation should be good via Mock OCSP');
      expect(sig.revocationStatus.source, equals('ocsp'),
          reason: 'Revocation source should indicate OCSP evidence');
    });

    test('Validate Signature Chain Trust (Failure Case - Missing Root)',
        () async {
      final pdfBytes = await generateSignedPdf();
    ...
    test('Sign Multiple PDFs in Parallel with External Signer', () async {
      final futures = List.generate(3, (index) async {
        ...
        if (!sigReport.validation.cmsSignatureValid ||
          ...
          throw Exception(
              'Validation failed for document $index: Intact=${sigReport.validation.documentIntact} CMS=${sigReport.validation.cmsSignatureValid} Trusted=${sigReport.chainTrusted} Rev=${sigReport.revocationStatus.status}');
        }
