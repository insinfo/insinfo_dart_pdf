...
void main() {
  group('Expanded Real World Scenarios', () {
    test(
      ...
      doc.pages.add().graphics.drawString('GovBr-Style 4-Level Chain Test',
          PdfStandardFont(PdfFontFamily.helvetica, 12),
          bounds: Rect.fromLTWH(0, 0, 300, 50));
      ...
      // 4. Compute Digest
      final digestBytes = _computeByteRangeDigest(
          prepared.preparedPdfBytes, prepared.byteRange);
    ...
    test('Scenario 2: Simple 2-Level Chain', () async {
      ...
      doc.pages.add().graphics.drawString(
          'Simple 2-Level Chain', PdfStandardFont(PdfFontFamily.helvetica, 12));
      final pdfBytes = Uint8List.fromList(await doc.save());
      doc.dispose();
    ...
    test('Diagnostic: Verify Chain Linkage (DN and KeyID)', () {
      ...
      expect(_bytesToHex(userIssuerRaw), equals(_bytesToHex(rootSubjectRaw)),
      ...
      final rootSKI =
          _getExtensionValue(rootExts, '2.5.29.14'); // SubjectKeyIdentifier
      final userAKI =
          _getExtensionValue(userExts, '2.5.29.35'); // AuthorityKeyIdentifier
      ...
      expect(_bytesToHex(Uint8List.fromList(keyIdFromAki)),
...
Future<void> _exportChainAsP7b(
    List<Uint8List> certsDer, String filePath) async {
  ...
  encapContent.add(
      ASN1ObjectIdentifier.fromComponentString('1.2.840.113549.1.7.1')); // data
  ...
  contentInfo.add(ASN1ObjectIdentifier.fromComponentString(
  ...
  for (var el in extensions.elements) {
    if (el is ASN1Sequence) {
      ...
      if (oid.identifier == oidStr) {
        final octet = el.elements[1] is ASN1OctetString
            ? el.elements[1] as ASN1OctetString
            :
            ...
            (el.elements[1] is ASN1Boolean
                ? el.elements[2] as ASN1OctetString
                : el.elements[1] as ASN1OctetString);
    ...
    if (!isSelfSigned) {
      ...
      if (cert.aki != null) {
        ...
        expect(issuerCert, isNotEmpty,
            reason:
                'Cert "${cert.cn}" with AKI=${cert.aki} has no matching issuer SKI in chain. '
...
void _validateCertificateExtensions(ASN1Sequence cert) {
  ...
  expect(extensionsWrapped, isNotNull,
      reason: 'Certificate must have Extensions [3]');
  ...
  final issuerBytes = tbs.elements[3].encodedBytes;
  final subjectBytes = tbs
      .elements[5].encodedBytes; // Adjust index if explicit tags shift things?
  ...
  expect(hasSKI, isTrue,
      reason: 'Certificate missing SubjectKeyIdentifier (2.5.29.14)');

  if (!isSelfSigned) {
    expect(hasAKI, isTrue,
        reason: 'Certificate missing AuthorityKeyIdentifier (2.5.29.35)');
  }
}
