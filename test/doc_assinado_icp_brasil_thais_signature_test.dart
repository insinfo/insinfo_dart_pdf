...
void main() {
  group('doc_assinado_icp_brasil_thais.pdf', () {
    test('extracts DER CMS and validates ICP-Brasil signature', () async {
      final File file = File('test/assets/doc_assinado_icp_brasil_thais.pdf');
      ...
      expect(sig.signerInfo?.cpf, '85936480704');
    });

    test('exposes creator and signer software metadata in raw PDF', () {
      final File file = File('test/assets/doc_assinado_icp_brasil_thais.pdf');
      expect(file.existsSync(), isTrue,
          reason: 'Arquivo não encontrado: ${file.path}');

      final String text =
          latin1.decode(file.readAsBytesSync(), allowInvalid: true);
