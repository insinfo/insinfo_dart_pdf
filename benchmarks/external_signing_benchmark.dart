  ...
  group('PdfExternalSigning Benchmark', () {
    ...
    setUpAll(() async {
      ...
      try {
        ...
        pdfBytes = inputBytes;
        print(
            'Warning: Failed to prepare PDF using PdfExternalSigning. Using raw bytes. Error: $e');
      }
    });
    ...
    test('Benchmark extractByteRange', () {
      ...
      try {
        ...
        print(
            'extractByteRange (Regex): ${regexTime / 1000} ms for $iterations iterations');
        ...
        print(
            'extractByteRange (FastBytes): ${fastTime / 1000} ms for $iterations iterations');
        ...
        print(
            'extractByteRange (InternalDoc): ${parserTime / 1000} ms for $iterations iterations');
        print('  Average: ${parserTime / iterations} µs/op');

        if (regexTime > 0) {
          final ratio = parserTime / regexTime;
          print(
              'Result: InternalDoc is ${ratio.toStringAsFixed(2)}x slower than Regex');
        }
        if (fastTime > 0) {
          final ratio = parserTime / fastTime;
          print(
              'Result: InternalDoc is ${ratio.toStringAsFixed(2)}x slower than FastBytes');
        }
      } catch (e) {
    ...
    test('Benchmark findContentsRange', () {
      ...
      try {
        ...
        print(
            'findContentsRange (String Search): ${stringSearchTime / 1000} ms for $iterations iterations');
        ...
        final fastTime = stopwatch.elapsedMicroseconds;
        print(
            'findContentsRange (FastBytes): ${fastTime / 1000} ms for $iterations iterations');
        print('  Average: ${fastTime / iterations} µs/op');
        ...
        final parserTime = stopwatch.elapsedMicroseconds;
        print(
            'findContentsRange (InternalDoc): ${parserTime / 1000} ms for $iterations iterations');
        print('  Average: ${parserTime / iterations} µs/op');

        if (stringSearchTime > 0) {
          final ratio = parserTime / stringSearchTime;
          print(
              'Result: InternalDoc is ${ratio.toStringAsFixed(2)}x slower than String Search');
        }
        if (fastTime > 0) {
          final ratio = parserTime / fastTime;
          print(
